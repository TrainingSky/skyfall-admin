...
export default function WallOfSuccess() {
  ...
  const handleSave = async () => {
    ...
    if (!data.heading) return toast.error("Heading is required");
    if (!data.body)    return toast.error("Body is required");
    if (!data.name)    return toast.error("Name is required");
    if (!data.role)    return toast.error("Role is required");
    if (!data.url)     return toast.error("Website URL is required");
    if (mode === "create" && !data.avatar) return toast.error("Avatar is required");
  ...
  return (
    <>
      ...
      <div className="layout">
        ...
        <main className="main">
          ...
          <div className="page-header">
            ...
            <button className="btn-add" onClick={openCreate}>＋ Add Story</button>
          </div>
          ...
          {loading ? (
            ...
            <div className="empty">
              ...
              <div className="empty-text">No success stories yet.</div>
            </div>
          ) : (
            <div className="cards-grid">
              {items.map((item) => (
                <div className="card" key={item._id}>
                  ...
                  <div className="testimonial-heading">{item.heading}</div>
      ...
      {modal && (
        <Modal
          title={modal.mode === "create" ? "New Story" : "Edit Story"}
          fields={FIELDS}
          data={modal.data}
